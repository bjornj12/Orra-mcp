...
export function createServer(projectRoot: string): { server: McpServer; manager: AgentManager } {
  ...
  server.tool("orra_kill", "Stop a bg session and optionally remove it + close PR. Use `claude attach <shortId>` to unblock waiting agents.", orraKillSchema.shape, async (args) => gate(() => handleOrraKill(manager, orraKillSchema.parse(args))));
  ...
  server.tool("orra_spawn", "Spawn a background agent via `claude --bg`. Records provenance in .orra/spawns/.", orraSpawnSchema.shape, async (args) => gate(() => handleOrraSpawn(manager, orraSpawnSchema.parse(args))));
